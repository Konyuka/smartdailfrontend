    ...
    <div class="md:flex flex-col md:flex-row md:min-h-screen">
      <div class="flex flex-col w-full md:w-84 text-gray-700 bg-indigo-900 dark-mode:text-gray-200 dark-mode:bg-gray-800 flex-shrink-0" x-data="{ open: false }">
        <div class="flex-shrink-0 px-2 py-2 flex flex-row-reverse">
          <button  @click="closePanel" class="ml-1 flex items-center justify-center h-10 w-10 rounded-full focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
        ...
        <nav  class="flex-grow md:block px-4 pb-4 md:pb-0 md:overflow-y-auto static">
          ...
          <div class="mt-3 text-center sm:mt-5">
            ...
            <div v-if="ongoingCall" class="mt-5">
                ...
                <svg class="ml-28 h-8 w-8 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="mt-5 sm:mt-6 text-center absolute bottom-24 left-24">
            <div class="">
            <!-- <button v-if="!callOn" @click='dial' type="button" class="inline-flex justify-center rounded-full border border-transparent shadow-sm px-4 py-4 bg-green-500 text-base font-medium text-black hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:col-start-2 sm:text-sm">
            ...
            <!-- hang -->
            <button @click='hangUp' :class="{'hidden': callOn == false, 'block': callOn == true}" type="button" class="inline-flex justify-center rounded-full border border-transparent shadow-sm px-4 py-4 bg-red-500 text-base font-medium text-white hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:col-start-2 sm:text-sm">
    ...
    methods: {
      ...
      dial() {
        ...
        if(this.number !== null && /^\d+$/.test(this.number)){
          ...
          return this.$http
            .post("/api/v1/dial/manual",payload, { headers: { "Content-Type": "application/json","Accept": "application/json", "Authorization": `Bearer ${localStorage.getItem('token')}` } })
            .then((response) => {
        ...
        let payload = { "username":localStorage.getItem('user'),  "phone": localStorage.getItem('phone'), "campaign": this.$store.state.campaign,"callerid": localStorage.getItem('callerid') };
        //console.log(payload)
        // Hit the grab endpoint 
        return this.$http.post("/api/v1/dial/grab",payload, { headers:  { "Content-Type": "application/json","Accept": "application/json", "Authorization": `Bearer ${localStorage.getItem('token')}` } })
        .then((response) => {
            if(response){
              true
            }})
      ...
      grabTransferred(){
        ...
        return this.$http
        .post("/api/v1/dial/grab",payload, { headers: { "Content-Type": "application/json","Accept": "application/json","Authorization": `Bearer ${localStorage.getItem('token')}` } })
        .then((response) => {
          if(response){
            true
          }})
      ...
      leave3Way(){
        ...
        let payload = {"username":      localStorage.getItem('user'),  "phone":         localStorage.getItem('phone'),"campaign":      this.$store.state.campaign, "callerid":      localStorage.getItem('callerid'), "phone_number":  localStorage.getItem('numberCalled'), "lead_id":       localStorage.getItem('lead_id'),}
        
        //Hit the endpoint with the above payload 
        return this.$http
        .post("/api/v1/transfer/leave3way",payload, { headers: {  "Content-Type": "application/json","Accept": "application/json", "Authorization": `Bearer ${localStorage.getItem('token')}` }})
        .then((response) => {
          if(response){
            true
          }
      ...
      XferLine(){
        let payload = { "username":localStorage.getItem('user'),"phone": localStorage.getItem('phone'), "campaign": this.$store.state.campaign,"agent": this.$store.state.addedNumber,}
       return this.$http
        .post("/api/v1/transfer/hangxfer",payload, { headers: {  "Content-Type": "application/json","Accept": "application/json", "Authorization": `Bearer ${localStorage.getItem('token')}` }})
        .then((response) => {
          if(response){
            this.$parent.onCallTrue = true
            this.$parent.grabTransfer = true
      ...
      parkCall() {
        ...
        return this.$http.post("/api/v1/dial/park",payload, { headers: { "Content-Type": "application/json","Accept": "application/json","Authorization": `Bearer ${localStorage.getItem('token')}` } })
          .then((response) => {
            if(response){
              true
            }})
      ...
      hangUp() {
          ...
          return this.$http
            .post("/api/v1/dial/hangup",payload, { headers: {  "Content-Type": "application/json", "Accept": "application/json","Authorization": `Bearer ${localStorage.getItem('token')}` } })
            .then((response) => {
              if(response){
                this.$parent.dispositionModal = true
                this.$parent.onCallTrue = false
      ...
      start() {
        this.timer = setInterval(() => { this.elapsedTime += 1000;}, 1000);
      },
      stop() {
